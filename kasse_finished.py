...
def format_decimal(amount):
    """Formatiert einen Decimal-Betrag auf zwei Dezimalstellen."""
...
def check_balance(chip_id):
    ...
    if result:
        ...
        print(f"Aktueller Kontostand: {format_decimal(balance)}")
    else:
...
def increase_balance(chip_id, amount):
    ...
    conn.commit()
    print(f"Kontostand für Chip-ID {chip_id} um {format_decimal(amount)} erhöht.")
...
def purchase_item(chip_id, amount):
    ...
    if result:
        ...
        if balance >= amount:
            ...
            conn.commit()
            print(f"Kauf erfolgreich! {format_decimal(amount)} wurde abgezogen. Neuer Kontostand: {format_decimal(balance - amount)}")
        else:
